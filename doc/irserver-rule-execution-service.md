...
```powershell
# Example: az group create --name inrule-prod-rg --location eus
...
```powershell
# Example: az appservice plan create --name inrule-prod-sp --resource-group inrule-prod-rg --location eus
...
```powershell
# Example: curl.exe -T .\InRuleLicense.xml -u "$($creds.userName):$($creds.userPWD)" "$($creds.publishUrl)/"
curl.exe -T LICENSE_FILE_PATH -u "$($creds.userName):$($creds.userPWD)" "$($creds.publishUrl)/"
```
